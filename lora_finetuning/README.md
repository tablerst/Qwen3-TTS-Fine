...
从 `qwen_tts/core/models/modeling_qwen3_tts.py` 与 `finetuning/sft_12hz.py` 来看，LoRA 最适合优先接在 **主 `talker` 主干** 的线性层上；`code_predictor` 属于更敏感的后续增强项，不建议第一阶段默认一起训练。
...
这样做的目标不是“改得越多越好”，而是先尽量避免把 `talker.code_predictor` 一起训偏，降低杂音、电流音、发词异常和整体失真的风险。

### 第二阶段可选增强模块

如果第一阶段已经稳定，但 **loss 降不动**、目标音色偏移不足、说话风格变化不明显，可以再分阶段增强：
...
3. 只有在主 `talker` 已稳定、风格迁移仍明显不足时，才评估：
   - `target_scope: talker_and_code_predictor`
...
7. 每个 epoch 跑一次 validation loss；
8. 保存 `best-checkpoint`，不要默认只看最后一个 epoch；
9. 推理前再把这些增量挂回基座模型。
...
- lr scheduler：`linear` 或 `cosine`
- weight decay：`0.0`
- validation split：`0.1` 左右，或单独准备固定 `validation_jsonl`
